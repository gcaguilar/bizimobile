...
class ReverseGeocodeUseCase(
    ...
    private suspend fun fallbackToGoogleReverseGeocode(location: GeoPoint, originalError: Throwable): GeoResult? {
        val apiKey = googleMapsApiKey?.takeIf { it.isNotBlank() }
        if (apiKey == null) {
            println("[GeoReverse] datosbizi reverse geocode failed without Google fallback: ${originalError::class.simpleName} ${originalError.message}")
            return null
        }

        println("[GeoReverse] Falling back to Google reverse geocode after ${originalError::class.simpleName}: ${originalError.message}")
