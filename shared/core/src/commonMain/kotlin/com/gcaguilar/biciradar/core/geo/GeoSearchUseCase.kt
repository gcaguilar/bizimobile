...
class GeoSearchUseCase(
  ...
  private suspend fun fallbackToGooglePlaces(
    query: String,
    ...
    val predictions =
      googlePlacesApi
        .autocomplete(query, biasLocation = null, apiKey = apiKey)
        ...
        name =
          details.name.ifBlank {
            prediction.description
              .substringBefore(',')
              .trim()
              .ifBlank { prediction.description }
          },
...
private fun deduplicateResults(results: List<GeoResult>): List<GeoResult> =
  results.distinctBy { geoResult ->
    geoResult.name.trim().lowercase() to geoResult.address.trim().lowercase()
  }

private fun normalizeSearchKey(query: String): String =
  query
    .replace(Regex("\\s+"), " ")
    .replace(Regex("\\s*,\\s*"), ", ")
    .trim()
