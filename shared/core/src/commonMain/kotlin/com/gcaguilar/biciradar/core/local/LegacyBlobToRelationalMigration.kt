...
object LegacyBlobToRelationalMigration {
  fun ensure(driver: SqlDriver, database: BiciRadarDatabase, json: Json) {
  ...
  private fun migrateSettingsBlobIfPresent(driver: SqlDriver, json: Json, db: BiciRadarDatabase) {
    val blobJson = readSingleStringColumn(driver, "settings_snapshot", "snapshot_json")
  ...
  private fun migrateSurfaceBlobIfPresent(driver: SqlDriver, json: Json, db: BiciRadarDatabase) {
    val blobJson = readSingleStringColumn(driver, "surface_snapshot_bundle", "bundle_json")
  ...
  private fun migrateSavedPlaceAlertsIfBlobgy(driver: SqlDriver, json: Json, db: BiciRadarDatabase) {
    ...
    rows.forEach { legacy ->
      val target = runCatching { json.decodeFromString<SavedPlaceAlertTarget>(legacy.targetJson) }.getOrNull() ?: return@forEach
      val condition = runCatching { json.decodeFromString<SavedPlaceAlertCondition>(legacy.conditionJson) }.getOrNull() ?: return@forEach
  ...
  private fun readSingleStringColumn(driver: SqlDriver, table: String, column: String): String? {
...
internal fun settingsSnapshotFromDbRow(row: App_settings, mapFilterNames: Set<String>): SettingsSnapshot =
  SettingsSnapshot(
...
@Suppress("LongMethod")
internal fun upsertSettingsFromSnapshot(db: BiciRadarDatabase, snapshot: SettingsSnapshot) {
...
private fun persistSurfaceBundle(db: BiciRadarDatabase, bundle: SurfaceSnapshotBundle) {
  val st = bundle.state
...
private fun insertStationRow(db: BiciRadarDatabase, role: String, slotIndex: Int, s: SurfaceStationSnapshot) {
...
internal fun surfaceBundleFromRows(
  ...
  fun mapRow(r: Surface_station_row) = SurfaceStationSnapshot(
    id = r.station_id,
  ...
  val monitoring = monRow?.station_id?.let { sid ->
...
internal fun persistSurfaceBundleRelational(db: BiciRadarDatabase, bundle: SurfaceSnapshotBundle) {
  persistSurfaceBundle(db, bundle)
}

private fun savedPlaceKindToDb(kind: SavedPlaceKind): String = kind.name

private fun conditionToKind(c: SavedPlaceAlertCondition): String = when (c) {
...
private fun conditionToThreshold(c: SavedPlaceAlertCondition): Long? = when (c) {
