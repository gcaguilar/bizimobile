...
actual class PlatformKeyPair(private val alias: String) {
    ...
    actual val publicKeyDerBase64: String
        get() {
            val pub = JvmKeyStore.getPublicKey(alias)
    ...
    actual fun sign(data: ByteArray): String {
        val priv = JvmKeyStore.getPrivateKey(alias)
...
private object JvmKeyStore {
    ...
    fun store(alias: String, priv: PrivateKey, pub: PublicKey) {
    ...
    fun delete(alias: String) { privateKeys.remove(alias); publicKeys.remove(alias) }
}
