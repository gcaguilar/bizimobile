...
class GeoFallbackUseCaseTest {
    ...
    @Test
    fun `geo search falls back to Google Places when datosbizi search fails`() = runTest {
        val useCase = GeoSearchUseCase(
            geoApi = object : GeoApi {
    ...
    @Test
    fun `geo search prioritizes Zaragoza-normalized variants for plaza espana`() = runTest {
    ...
    @Test
    fun `reverse geocode falls back to Google geocoding when datosbizi reverse fails`() = runTest {
        val location = GeoPoint(41.6492, -0.8833)
        val useCase = ReverseGeocodeUseCase(
            geoApi = object : GeoApi {
    ...
    @Test
    fun `reverse geocode returns null when datosbizi fails and no Google key is configured`() = runTest {
        val useCase = ReverseGeocodeUseCase(
            geoApi = object : GeoApi {
                override suspend fun search(query: String): List<GeoResult> = emptyList()
    ...
    private class FakeGooglePlacesApi(
        ...
        override suspend fun placeDetails(placeId: String, apiKey: String): PlaceDetails? = detailsById[placeId]

        override suspend fun reverseGeocode(location: GeoPoint, apiKey: String): String? = reverseGeocodeAddress
