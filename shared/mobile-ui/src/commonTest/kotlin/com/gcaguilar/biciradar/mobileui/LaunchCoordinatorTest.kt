...
class LaunchCoordinatorTest {
  @Test
  fun `mobile select city reuses shared city change semantics`() =
    runTest {
      ...
      val resolution =
        coordinator.resolveMobileLaunch(
  ...
  @Test
  fun `assistant search station keeps query and routes to detail when matched`() =
    runTest {
      ...
      val resolution =
        coordinator.resolveAssistantLaunch(
