...
class ShortcutsViewModelTest {
  ...
  @Test
  fun `resolves assistant action into latest answer`() =
    runTest(dispatcher) {
      val viewModel =
        ShortcutsViewModel(
          assistantIntentResolver = RecordingAssistantIntentResolver(),
          stationsRepository =
            FakeShortcutsStationsRepository(
              StationsState(
                stations =
                  listOf(
                    Station(
  ...
  @Test
  fun `latest request result wins when older request finishes later`() =
    runTest(dispatcher) {
...
private class FakeShortcutsFavoritesRepository(
