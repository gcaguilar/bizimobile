...
internal fun FavoritePill(
    ...
    val scale by animateFloatAsState(
        ...
        animationSpec = androidx.compose.animation.core.spring(dampingRatio = 0.78f, stiffness = 720f),
        label = "favorite-pill-scale",
    )
    Surface(
        shape = RoundedCornerShape(16.dp),
        color = LocalBiziColors.current.red.copy(alpha = 0.10f * containerColor),
        border = BorderStroke(1.dp, if (active) LocalBiziColors.current.red.copy(alpha = 0.16f) else LocalBiziColors.current.panel),
        modifier = Modifier
            .graphicsLayer {
                scaleX = scale
                scaleY = scale
...
internal fun OutlineActionPill(
    ...
    Surface(
        ...
        Row(
            ...
            Text(label, color = tint, style = MaterialTheme.typography.labelMedium, fontWeight = androidx.compose.ui.text.font.FontWeight.SemiBold)
        }
    }
}
