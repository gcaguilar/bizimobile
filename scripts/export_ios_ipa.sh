...
archive_path="${2:-${RUNNER_TEMP:-/tmp}/BiziZaragoza.xcarchive}"
project_path="${APPLE_XCODE_PROJECT_PATH:-apple/BiziZaragoza.xcodeproj}"
scheme_name="${APPLE_XCODE_SCHEME:-BiziZaragoza}"
bundle_identifier="${APPLE_BUNDLE_ID:-com.gcaguilar.bizizaragoza.ios}"
export_method="${APPLE_EXPORT_METHOD:-debugging}"
signing_certificate="${APPLE_SIGNING_CERTIFICATE_TYPE:-}"
export_options_plist="${RUNNER_TEMP:-/tmp}/BiziZaragoza-export-options.plist"
