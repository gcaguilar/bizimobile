...
class GarminPayloadBuilder {
    ...
    private fun Station.toGarminData() = GarminStationData(
        id = id,
    ...
    fun serializeToJson(payload: GarminStationsPayload): String {
        return buildString {
            ...
            append(",\"backup\":[")
            append(payload.backup.joinToString(",") { station ->
...
class GarminBleManager(
    ...
    private fun getConnectedDevices(): List<Any> {
        return try {
            ...
            val getInstanceMethod = connectIQClass.getMethod(
                "getInstance",
                Context::class.java,
    ...
    private fun sendMessageToDevice(device: Any, message: String) {
        try {
            val connectIQClass = Class.forName("com.garmin.connectiq.ConnectIQ")
            val getInstanceMethod = connectIQClass.getMethod(
                "getInstance",
            ...
            val sendMessageMethod = connectIQ.javaClass.getMethod(
                "sendMessage",
                device.javaClass,
